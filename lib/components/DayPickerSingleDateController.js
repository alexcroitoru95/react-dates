...
function (_ref) {
  ...
  _proto.componentWillReceiveProps = function componentWillReceiveProps(nextProps) {
    ...
    var didDatesChange = !_.isEqual(dates, prevDates);
    ...
    if (didDatesChange) {
      var newDates = dates.filter(function (newDate) {
        return prevDates.filter(function (prevDate) {
          return (0, _moment["default"])(newDate).format(MOMENT_FORMAT) !== (0, _moment["default"])(prevDate).format(MOMENT_FORMAT);
        });
      });

      if (newDates.length > 0) {
        newDates.map(function (item) {
          modifiers = _this2.addModifier(modifiers, item, 'selected');
        });
      }
    }
