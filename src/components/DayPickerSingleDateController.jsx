...
export default class DayPickerSingleDateController extends React.PureComponent {
  ...
  componentWillReceiveProps(nextProps) {
    ...
    if (didDatesChange) {
      dates.map((newerDate) => {
        prevDates.map((previousDate) => {
          if (
            moment(newerDate).format(MOMENT_FORMAT)
                        !== moment(previousDate).format(MOMENT_FORMAT)
          ) {
  ...
  isLastDayOfWeek(day) {
    ...
    return (
      day.day()
            === ((firstDayOfWeek || moment.localeData().firstDayOfWeek()) + 6) % 7
    );
  }
